...
RESUME_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Teja_Gundeti_Resume.pdf")
...
if os.path.exists(RESUME_PATH) and st.session_state.retrieval_chain is None:
    with st.spinner("⏳ Processing Teja's data..."):
        # Load and process the document (with fallback for slightly malformed PDFs)
        docs = []
        try:
            loader = PyPDFLoader(RESUME_PATH, extract_images=False)
            docs = loader.load()
        except Exception:
            pass

        # If loader failed or returned empty text, try pypdf strict=False then pdfplumber
        if not docs or all(len(d.page_content.strip()) == 0 for d in docs):
            import pypdf, pdfplumber
            from langchain_core.documents import Document
            try:
                reader = pypdf.PdfReader(RESUME_PATH, strict=False)
                docs = [Document(page_content=page.extract_text() or "", metadata={"page": i})
        ...
        # Final fallback: use hardcoded resume text if PDF is unreadable
        if not docs or all(len(d.page_content.strip()) == 0 for d in docs):
            from langchain_core.documents import Document
            resume_text = """
Teja Gundeti - Full Stack Developer & Data Analyst
Email: tejagundeti7@gmail.com | LinkedIn: linkedin.com/in/tejagundeti | GitHub: github.com/tejagundeti
...
Data Analysis: Python (Pandas, NumPy, Matplotlib, Seaborn), SQL, Power BI, Looker Studio
Tools: Git, GitHub, VS Code, Postman, Docker (basics)
Cloud: AWS (basics), Firebase, Streamlit

PROJECTS
1. BachelorsPay - A fintech web app for bachelors to manage shared expenses, loans, and payments.
   Tech: HTML, CSS, JavaScript, Firebase, Chart.js
   Features: Room Dashboard, Budget Tracker, Transaction History, Scan & Pay, AI Insights

2. AI Resume Assistant - A Streamlit chatbot powered by Groq LLM and LangChain RAG to answer questions about Teja's resume.
   Tech: Python, Streamlit, LangChain, FAISS, HuggingFace Embeddings, Groq API

3. Data Analysis Dashboard - Analyzed datasets using Python and created interactive visualizations.
   Tech: Python, Pandas, Matplotlib, Power BI, Looker Studio

4. Full Stack Web Applications - Built multiple CRUD web apps during training and academics.
   Tech: React.js, Node.js, Express.js, MongoDB, MySQL

INTERNSHIP / TRAINING
- Full Stack Development Training (React.js, Node.js, MongoDB)
- Data Analysis Training (Python, SQL, Power BI)

STRENGTHS
- Fast learner and quick adapter to new technologies
- Strong problem-solving and analytical skills
- Passionate about building impactful web applications
- Team player with good communication skills
- Fresher eager to contribute and grow in a real-world environment

ACHIEVEMENTS
- Built BachelorsPay, a complete fintech prototype from scratch
- Created an AI-powered resume chatbot deployed on Streamlit Cloud
- Consistently demonstrated initiative through self-driven projects
            """
            docs = [Document(page_content=resume_text, metadata={"source": "hardcoded_resume"})]
    ...
    """
        ...
        # Create LCEL chain (replaces deprecated create_stuff_documents_chain / create_retrieval_chain)
...
if st.session_state.thinking and st.session_state.retrieval_chain is not None:
    # Generate response
    answer = st.session_state.retrieval_chain.invoke(st.session_state.messages[-1]["content"])
...
st.markdown("---")
st.markdown("👨‍💻 Developed with ❤️ using Groq, LangChain & Streamlit")